...
hosts the operational-state REST plane (checkpoints + leases on DynamoDB,
separate run-ledger table, S3 packet archive). **`axon-service`** ([`backend/axon-service/`](backend/axon-service/README.md)) is the graph retrieval plane: `/axon/{company}/{repo}/index|query|impact` and `/healthz` with S3 + DynamoDB backing; set **`AXON_SERVICE_URL`** (base URL for `canon memory-health` graph), **`AXON_S3_BUCKET`**, **`AXON_META_TABLE_NAME`**, and **`AXON_SERVICE_TOKEN`**. The layout also includes v2 libs **`knowledge-schema`**,
...
| `canon flow-audit --handoff-id <id> --task-id <id> [--require-checkpoints] [--require-memory-health] [--require-deploy-attestation]` | Audit process compliance artifacts (handoff files + plan/task tracking), with optional sampling; **`--require-checkpoints`** enforces per-phase checkpoint JSON; **`--require-memory-health`** requires on-disk `memory-health.json`; **`--require-deploy-attestation`** requires `deployment-smoke.json` under the task handoff dir (schema enforced in `flow_audit.py` — no secret payloads). |
...
| `canon packet-archive --file <path> ... [--dry-run]` | Upload one packet/evidence file to **`STATE_ARTIFACT_BUCKET`** via **`POST /state/archive`** (requires `STATE_ARTIFACT_BUCKET` on the server). **`--dry-run`** resolves the archive record JSON locally without network I/O. Does not remove the requirement for local `.cursor/handoffs/...` files. |
| `canon run-ledger --record-file <path> \| --record-json '<obj>' [--merge-archive-json <path>] [--dry-run] [--state-api-url ...]` | Validate/normalize a v1 run-ledger record; optionally merge archive **metadata** into `archive_refs` (no bodies). **`--dry-run`** prints JSON only; otherwise **`PUT`**s to **`/state/run-ledger`** (requires **`STATE_RUN_LEDGER_TABLE_NAME`** on `state-api`). Does not encode merge policy by itself. |
| `canon readiness check --company-id <c> --repository-id <r> --plan-id <p> --task-id <t> --workstream-id <w> --handoff-id <h> [--ledger-run-id <id>] [--state-api-url <url>] [--limit N] [--output <path>] [--quiet]` | **Read-only** diagnosis over **`GET` `/state/run-ledger`**: JSON snapshot from ledger rows + `archive_refs` (no body reads); optional **`--output`** mirrors **`readiness.json`** under handoffs; exits **`0`** if **ready**, **`1`** if evaluated **not ready**, **`2`** on usage / query errors. Summarizes **`validation_outcomes`** / deployment fields already on the row; does **not** normalize QA evidence, refactor DoR, or invent new attestation rules — use **`canon qa-validate`**, **`canon flow-audit`** (including **`--require-deploy-attestation`** for **`deployment-smoke.json`**), **`canon doctor`**, and templates for policy — see **`docs/SYSTEM-WORKFLOW.md` §3**. |

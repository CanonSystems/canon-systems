...
from state_api.storage import RunLedgerStore, StateStore, TasksStore
from state_api.tasks import get_tasks_store

TABLE = "test-canon-state"
LEDGER_TABLE = "test-canon-run-ledger"
TASKS_TABLE = "test-canon-tasks"
...
def dynamodb_table() -> str:
    ...
    with mock_aws():
        ...
        for tbl in (TABLE, LEDGER_TABLE, TASKS_TABLE):
            client.create_table(
                TableName=tbl,
                KeySchema=[
...
def client(dynamodb_table: str, captured_events: list) -> TestClient:
    settings = Settings(
        ...
        state_tasks_table_name=TASKS_TABLE,
    ...
    tasks_store = TasksStore(TASKS_TABLE, "us-east-1")
    ...
    app.dependency_overrides[get_tasks_store] = ov_tasks

...
from canon_systems.version_check import _version_tuple, check
...
def test_check_ok_when_pin_equal(tmp_path: Path):
    ...
    env.write_text(f"CANON_SYSTEMS_VERSION={__version__}\n", encoding="utf-8")
    ok, _ = check(tmp_path)
    assert ok is True
...
def test_check_fails_when_installed_older_than_pin(tmp_path: Path):
    ...
    env.write_text("CANON_SYSTEMS_VERSION=999.0.0\n", encoding="utf-8")
    ok, msg = check(tmp_path)
    assert ok is False
    assert "pipx upgrade canon-systems" in msg


def test_check_ok_when_installed_newer_than_pin(tmp_path: Path):
    env = tmp_path / ".canon" / "memory-layer.local.env"
    env.parent.mkdir(parents=True)
    env.write_text("CANON_SYSTEMS_VERSION=0.0.1\n", encoding="utf-8")
    ok, _ = check(tmp_path)
    assert ok is True


def test_check_honors_legacy_pin_key(tmp_path: Path):
    """Repos wired with the old canon-memory-layer package still work:
    we read CANON_MEMORY_LAYER_VERSION as a fallback."""
    env = tmp_path / ".canon" / "memory-layer.local.env"
    env.parent.mkdir(parents=True)

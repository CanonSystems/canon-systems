...
| **Client cache TTL** | After hydration, decoded secret fields are mirrored to `~/.canon/memory-layer-aws-cache.json` with a **TTL** (default 15 minutes). Rotated secrets or wrong cached URLs → clear cache: `canon doctor --fix-cache` or `rm -f ~/.canon/memory-layer-aws-cache.json`. |
